...
static const struct sunxi_register sunxi_regs[] = {
...
{
	struct sunxi		*sunxi = s->private;
	int			i;

	seq_printf(s, "DesignWare USB3 Core Register Dump\n");

	for (i = 0; i < ARRAY_SIZE(sunxi_regs); i++) {
		seq_printf(s, "%-20s :    %08x\n", sunxi_regs[i].name,
				sunxi_readl(sunxi->regs, sunxi_regs[i].offset));
	}
...
int __devinit sunxi_debugfs_init(struct sunxi *sunxi)
{
	struct dentry		*root;
	struct dentry		*file;
	int			ret;

	root = debugfs_create_dir(dev_name(sunxi->dev), NULL);
	if (IS_ERR(root)) {
		ret = PTR_ERR(root);
		goto err0;
	}
	...
	file = debugfs_create_file("regdump", S_IRUGO, root, sunxi,
			&sunxi_regdump_fops);
	if (IS_ERR(file)) {
		ret = PTR_ERR(file);
		goto err1;
...
void __devexit sunxi_debugfs_exit(struct sunxi *sunxi)
{
	debugfs_remove_recursive(sunxi->root);
	sunxi->root = NULL;
}
#endif
