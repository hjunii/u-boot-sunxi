/*
 * drivers/usb/gadget/sunxi_udc_otg_xfer_dma.c
 */
...
static inline void sunxi_udc_ep0_zlp(struct sunxi_udc *dev)
{
#if 0
	u32 ep_ctrl;
...
static inline void sunxi_ep0_complete_out(void)
{
#if 0
	u32 ep_ctrl;
...
static int setdma_rx(struct sunxi_ep *ep, struct sunxi_request *req)
{
#if 0
	u32 *buf, ctrl;
	u32 length, pktcnt;
...
int setdma_tx(struct sunxi_ep *ep, struct sunxi_request *req)
{
#if 0
	u32 *buf, ctrl = 0;
	u32 length, pktcnt;
...
static void complete_rx(struct sunxi_udc *dev, u8 ep_num)
{
#if 0
	struct sunxi_ep *ep = &dev->ep[ep_num];
	struct sunxi_request *req = NULL;
...
static void complete_tx(struct sunxi_udc *dev, u8 ep_num)
{
#if 0
	struct sunxi_ep *ep = &dev->ep[ep_num];
...
static inline void sunxi_udc_check_tx_queue(struct sunxi_udc *dev, u8 ep_num)
{
#if 0
	struct sunxi_ep *ep = &dev->ep[ep_num];
	struct sunxi_request *req;
...
static void process_ep_in_intr(struct sunxi_udc *dev)
{
#if 0
...
static void process_ep_out_intr(struct sunxi_udc *dev)
{
#if 0
	u32 ep_intr, ep_intr_status;
	u8 ep_num = 0;
...
int sunxi_udc_get_status(struct sunxi_udc *dev,
...
void sunxi_udc_ep_set_stall(struct sunxi_ep *ep)
{
#if 0
	u8		ep_num;
	u32		ep_ctrl = 0;
...
void sunxi_udc_ep_clear_stall(struct sunxi_ep *ep)
{
#if 0
	u8		ep_num;
	u32		ep_ctrl = 0;
...
static int sunxi_udc_set_halt(struct usb_ep *_ep, int value)
{
	...
	if (value && ep_is_in(ep) && !list_empty(&ep->queue)) {
		debug("%s: %s queue not empty, req = %p\n",
			__func__, ep->ep.name,
			list_entry(ep->queue.next, struct sunxi_request, queue));
	...
	if (value == 0) {
		ep->stopped = 0;
		sunxi_udc_ep_clear_stall(ep);
	} else {
		if (ep_num == 0)
			dev->ep0state = WAIT_FOR_SETUP;

		ep->stopped = 1;
		sunxi_udc_ep_set_stall(ep);
	}
...
void sunxi_udc_ep_activate(struct sunxi_ep *ep)
{
#if 0
	u8 ep_num;
...
static int sunxi_udc_clear_feature(struct usb_ep *_ep)
{
#if 0
	struct sunxi_udc	*dev;
	struct sunxi_ep	*ep;
...
static int sunxi_udc_set_feature(struct usb_ep *_ep)
{
#if 0
	struct sunxi_udc	*dev;
	struct sunxi_ep	*ep;
...
void sunxi_ep0_setup(struct sunxi_udc *dev)
{
	...
	if (dev->req_std) {
		...
		case USB_REQ_GET_STATUS:
			if (!sunxi_udc_get_status(dev, usb_ctrl))
				return;
		...
		case USB_REQ_CLEAR_FEATURE:
			...
			if (!sunxi_udc_clear_feature(&dev->ep[ep_num].ep))
				return;
		...
		case USB_REQ_SET_FEATURE:
			...
			if (!sunxi_udc_set_feature(&dev->ep[ep_num].ep))
				return;
